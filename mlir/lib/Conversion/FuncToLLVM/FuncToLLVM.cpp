...
static constexpr StringRef barePtrAttrName = "func.bareptr";

/// Return `true` if the `op` should use base pointer calling convention.
static bool shouldUseBarePtrCallConv(Operation *op,
                                     LLVMTypeConverter *typeConverter) {
  return op && (op->hasAttr("func.bareptr") ||
...
/// the base pointer calling convention lowering of `memref` types.
...
struct ReturnOpLowering : public ConvertOpToLLVMPattern<func::ReturnOp> {
  ...
  matchAndRewrite(func::ReturnOp op, OpAdaptor adaptor,
    ...
    bool useBarePtrCallingConv =
        shouldUseBarePtrCallingConv(funcOp, getTypeConverter());
    if (useBarePtrCallingConv) {
